...
Conditional claim under BEGIN IMMEDIATE; backoff; dead-letter escalations persist.
...
python -m pytest -q tests/test_governance_outbox.py
python -m pytest -q
```

Result: **203 passed** (`python -m pytest -q`, 2026-07-24).

Evidence: Conditional claim under BEGIN IMMEDIATE; backoff; dead-letter escalations persist.

## Changed files

- `src/holodeck_governance/storage/sqlite/outbox.py`
- `tests/test_governance_outbox.py`

## Residual risks

- Delivery workers beyond claim/retry/escalation remain adapter-side.
